...
import { useUpdateEventMutation } from "../../api/eventsApiSlice"
...
const FormEditEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const validationIsClear = validateInput(postData)

    if (validationIsClear) {
      try {
        await updateEvent(postData).unwrap()
      ...
      validationMessages = getValidationMessage(validationIsClear)
    }
  }
  ...
  const onStartingTimeChanged = (value) =>
    setNewEvent({ ...newEvent, startingTime: value })

  const onEndingTimeChanged = (value) =>
    setNewEvent({ ...newEvent, endingTime: value })

  const onChoosingImage = ({ base64 }) => {
    setNewEvent({ ...newEvent, image: base64 })
    setIsImageDeleted(false)
  }
  const onColorChanged = (moodColor) =>
    setNewEvent({ ...newEvent, colorMood: moodColor })
